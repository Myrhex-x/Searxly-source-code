...
//  Searxly
//
//  Full-page translation via Apple's Translation framework — entirely ON-DEVICE (macOS 15+).
//  No page text ever leaves the Mac: the framework runs local models (macOS downloads a language
//  pack from Apple the first time a pair is used — a system asset fetch, never the page content).
//
//  How it works, per translate:
//    1. A JS pass walks the page's visible text nodes, remembers each node + its original text in
//       page-world state (`window.__searxlyTx`), and returns the strings.
//    2. The strings run through a TranslationSession in batches; each finished batch is injected
//       straight back into its nodes (the page visibly translates progressively).
//    3. "Show Original" restores the remembered originals — no reload, no state lost.
//
//  The SwiftUI `.translationTask` modifier owns the session (ContentView attaches it): setting
//  `configuration` starts a run; the framework presents its own language-download UI when needed.
//
...
final class PageTranslator {
    ...
    /// Non-nil kicks off `.translationTask` in ContentView, which calls `run(_:)` with a session.
    ...
    /// Webview → URL it was translated on. A navigation makes the entry stale (URLs differ), so the
    /// ☰ menu's "Show Original" label resets to "Translate Page" without any lifecycle hooks.
    ...
    private func beginTranslation(of webView: WKWebView) {
        ...
        // Target = the one app-wide language choice (Settings → Search → Language), falling back to
        // the system language. Source nil → the framework detects the page's language itself.
        let target = AppLanguage.override.map { Locale.Language(identifier: $0) }
            ?? Locale.current.language

        // (Re)fire ContentView's `.translationTask`: a first/changed configuration fires on
        // assignment; an identical pair needs `invalidate()` to run again (Apple's documented pattern).
    ...
    /// Called by ContentView's `.translationTask` with a live session. Collects, translates in
    ...
    func run(_ session: TranslationSession) async {
        ...
        guard !strings.isEmpty else {
            notifyFailure("There's no translatable text on this page.")
            return
        }
        ...
        do {
            ...
            // Most common real cause: the page is already in the target language (unsupported pairing).
            notifyFailure("Couldn't translate this page — it may already be in your language, or the language pair isn't supported on-device.")
            return
        }
    ...
    private func notifyFailure(_ message: String) {
        NotificationManager.shared.show(
            ...
            body: message,
            source: "Searxly", iconSystemName: "character.bubble")
    }
    ...
    private func apply(pairs: [[Any]], to webView: WKWebView) async -> Bool {
        ...
        // same no-escaping-needed transport as the wallet bridge, made UTF-8-correct.
    ...
    /// a JSON array of their strings. Skips code/editable/invisible-ish content; caps size so a
    /// pathological page can't queue an hour of model work.
      ...
      try {
        ...
        var walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, {
          acceptNode: function (n) {
            ...
            // Needs at least one letter (any script) — pure numbers/punctuation don't translate.
